...
export class UserProvider {
	...
	private token: string;
  	...
  	*/
  	...
  	/*
		Return promise with List of users found by that key
  	*/
  	findUsers(keySearch, page){
  		...
  		return new Promise((resolve, reject) => {
  			...
  			this.storage.get('token').then(
	  			(token) => {
			  		...
			  		headers.append('Authorization', token);
			  		params.set('keySearch', keySearch);
			  		params.set('page', page);

			  		requestOptions.headers = headers;
	  				requestOptions.params = params;

			  		this.http.get(Constants.FIND_USERS_BY_KEY, requestOptions).subscribe(
			  			res => {
			  				resolve(res.json());
			  			},
			  			(err) => {
  	...
  	/*
		Update the restaurant of the loggedUser
  	*/
  	saveUserRestaurant(restaurant){
  		...
  		return new Promise((resolve, reject) => {
  			...
  			this.storage.get('token').then(
	  			(data) => {
			  		...
			  		this.http.post(Constants.SAVE_USER_RESTAURANT, JSON.stringify(restaurant), {headers: headers})
		  				.subscribe(
		  					res => {
